function [pwasys,pwactrl] = SyntGlblBMIElp(pwasys, setting)
...
if setting.NormalDirectionOnly
...
K_robust=robust_LQR_piecewise_quad(istar(1),pwasys,setting);
...
for i=istar
    for j=col_index
        P_11 = Q*(A{i,j}+B{i,j}*K{i})+(A{i,j}+B{i,j}*K{i})'*Q+alpha*Q;
        DV{i, j}=[P_11];
    end
end
...
for i=region_excluded_istar
    for j=col_index
        P_11 = Q*(A{i,j}+B{i,j}*K{i})+(A{i,j}+B{i,j}*K{i})'*Q+alpha*Q...
            +miu{i}*E{i}'*E{i};
        P_12 =Q*(a{i,j}+B{i,j}*k{i})+miu{i}*E{i}'*e{i};
        P_21 = P_12';
