function [reshaping, M]=CtrlContNormalDir(istar, pwasys)
...
S=zeros(L*Num_rltn*Num_S, NR*rgn_lngth);
...
for x=1:NR
    for y=x+1:NR
        if ~isempty(Fbar{x,y})
            ...
            row_vec=(rltn_index-1)*(L*Num_S)+1:rltn_index*(L*Num_S);
...
for envelope_index=col_index
    % synthesizing equivlent of cijBKF_{ij}
    A=cij'*pwasys.Bbar{x,envelope_index}(1:Num_S,:);
    X=rand(Num_I,Num_S);
    B=Fij;
    cijBKF_ij=local_multiplier_builder(A,X, B);
    ...
    %synthesizing cijBk_ij
    A=cij'*pwasys.Bbar{x,envelope_index}(1:Num_S,:);
    X=rand(Num_I,1);
    B=eye(1);
    cijBk_ij=local_multiplier_builder(A,X, B);
    
    %synthesizing S
    temp=[cijBKF_ij       zeros(size(cijBKF_ij,1), Num_I) zeros(size(cijBKF_ij,1),1);
          cijBKf_ij       cijBk_ij                        zeros(size(cijBKf_ij,1), 1)];
